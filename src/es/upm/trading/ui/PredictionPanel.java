...
public class PredictionPanel extends JPanel {
	...
	private void displayResult(PredictionResult res) {
		...
		lblChange.setForeground(pct > 0 ? new Color(60, 220, 100) : pct < 0 ? new Color(240, 80, 80) : Color.WHITE);
		...
		if(conf >= 0.80) {
			confDesc="Alta";
			colorConf=new Color(60, 220, 100);
		}
		else if(conf >= 0.50) {
			confDesc="Media";
			colorConf=new Color(255, 200, 60);
		}
		else {
			confDesc="Baja";
			colorConf=new Color(240, 120, 80);
		}
		lblConfidence.setText(String.format("%.1f%%  (%s)", conf * 100, confDesc));
		lblConfidence.setForeground(colorConf);
	}
